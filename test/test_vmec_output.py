...
class Test(unittest.TestCase):
        ...
        for itheta in range(0,self.vmecOutput.ntheta,5):
            for izeta in range(0,self.vmecOutput.nzeta,5):
                ...
                dxdzeta_fd = finite_difference_derivative(this_zeta,\
                    lambda zeta : self.vmecOutput.position(-1, \
                ...
                dxdtheta_fd = finite_difference_derivative(this_theta,\
                    lambda theta : self.vmecOutput.position(-1, \
    ...
    def test_position_second_derivatives(self):
        ...
        for itheta in range(0,self.vmecOutput.ntheta,5):
            for izeta in range(0,self.vmecOutput.nzeta,5):
                ...
                d2ydthetadzeta_fd = finite_difference_derivative(this_zeta,\
                    lambda zeta : self.vmecOutput.position_first_derivatives(-1, \
                    this_theta,zeta)[2], epsilon=1e-5 ,method='centered')
                self.assertAlmostEqual(d2ydthetadzeta_fd, \
                                       d2ydthetadzeta[izeta,itheta], places=5)
                d2zdthetadzeta_fd = finite_difference_derivative(this_zeta,\
                    lambda zeta : self.vmecOutput.position_first_derivatives(-1, \
    ...
    def test_jacobian_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            ...
            djacobiandrmnc_fd = finite_difference_derivative(this_rmnc,\
                lambda rmnc : temp_fun_rmnc(im,rmnc), epsilon=1e-5, \
                method='centered')
            djacobiandzmns_fd = finite_difference_derivative(this_zmns,\
    ...
    def test_radius_derivatives(self):
        ...
        def temp_fun_rmnc(im, rmnc):
            rmnc_init = self.vmecOutput.rmnc[-1, im]
            self.vmecOutput.rmnc[-1, im] = rmnc
            [X, Y, Z, R] = self.vmecOutput.position()
            self.vmecOutput.rmnc[-1, im] = rmnc_init
            return R
        ...
        for im in range(len(xm_sensitivity)):
            this_rmnc = self.vmecOutput.rmnc[-1, im]
            dRdrmnc_fd = finite_difference_derivative(this_rmnc,\
                lambda rmnc : temp_fun_rmnc(im, rmnc), epsilon=1e-5, \
                method='centered')
            for izeta in range(self.vmecOutput.nzeta):
                for itheta in range(self.vmecOutput.ntheta):
    ...
    def area_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rmnc = self.vmecOutput.rmnc[-1, im]
            this_zmns = self.vmecOutput.zmns[-1, im]
            dareadrmnc_fd = finite_difference_derivative(this_rmnc,\
                lambda rmnc : temp_fun_rmnc(im, rmnc), epsilon=1e-5, \
                method='centered')
            dareadzmns_fd = finite_difference_derivative(this_zmns,\
                lambda zmns : temp_fun_zmns(im, zmns), epsilon=1e-5, \
                method='centered')
    ...
    def test_normalized_jacobian_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rmnc = self.vmecOutput.rmnc[-1,im]
            this_zmns = self.vmecOutput.zmns[-1,im]
            dnormalized_jacobiandrmnc_fd = finite_difference_derivative(this_rmnc,\
                lambda rmnc : temp_fun_rmnc(im,rmnc), epsilon=1e-5, \
                method='centered')
            dnormalized_jacobiandzmns_fd = finite_difference_derivative(this_zmns,\
                lambda zmns : temp_fun_zmns(im,zmns), epsilon=1e-5, \
                method='centered')
            for izeta in range(self.vmecOutput.nzeta):
                for itheta in range(self.vmecOutput.ntheta):
    ...
    def test_B_on_arclength_grid(self):
        ...
        modB2 = self.vmecOutput.modB(isurf=self.vmecOutput.ns_half - 1, \
                                            full=False) ** 2 
    ...
    def test_modB(self):
        ...
        self.assertRaises(ValueError, self.vmecOutput.modB, \
    ...
    def test_current(self):
        ...
        """           
        It_half = self.vmecOutput.current()
    ...
    def test_N(self):
        ...
        self.assertRaises(ValueError, self.vmecOutput.N, \
                         theta = thetas, zeta = zetas)
        self.assertRaises(ValueError, self.vmecOutput.N, \
                         isurf = self.vmecOutput.ns)
        
    def test_iota_objective(self):
        """
        ...
        self.assertRaises(TypeError, self.vmecOutput.iota_objective, \
                         weight = weight)
        
    def test_well_objective(self):
        """
            Test that ValueError is raised if incorrect weight is passed.
        """                   
        weight = self.vmecOutput.s_half
        ...
        self.assertRaises(TypeError, self.vmecOutput.well_objective, \
                         weight = weight)
    ...
    def test_modB_objective(self):
        """
            Test that ValueError is raised if incorrect isurf is passed.
        """  
        self.assertRaises(ValueError, self.vmecOutput.modB_objective, \
                          isurf = self.vmecOutput.ns)

    def test_modB_objective_volume(self):
        """
            Test that a positive float is returned.
        """  
        output = self.vmecOutput.modB_objective_volume()
